...
**Dataset:** Cats and Dogs classification dataset — [CATS and Dogs binary classification dataset from Kaggle](https://www.kaggle.com/datasets/bhavikjikadara/dog-and-cat-classification-dataset)
...
1. **Automated Testing**
   ...
   - Write unit tests for at least one data pre-processing function and one model utility/inference function.
   - Ensure tests run via pytest or similar.
2. **CI Setup** (Choose one: GitHub Actions / GitLab CI / Jenkins / Tekton)

   Define a pipeline that on every push/merge request:
...
1. **Deployment Target**
   ...
   - Define infrastructure manifests:
     - For Kubernetes: Deployment + Service YAML.
...
1. A zip file containing all source code, configuration files (DVC, CI/CD, Docker, deployment manifests), and trained model artifacts.
