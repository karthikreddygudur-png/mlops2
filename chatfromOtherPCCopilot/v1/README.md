...
**This is normal. Do not abort it.** The long step is `pip install` fetching about
200 MB of wheels. Let it run to completion.

**Do NOT do any of the following to make the build "faster":**
- Do not remove or skip `torch` / `torchvision`. They are the model runtime. Without
  them the image builds but the service starts with `model_loaded: false` and every
  prediction returns HTTP 503 — Task 10 will fail.
- Do not switch to a different base image. `python:3.12-slim` is already the slim variant.
- Do not edit the `Dockerfile`. It is a multi-stage build that already discards build
  tooling and installs CPU-only torch wheels (~200 MB rather than ~2.5 GB for the CUDA
  build). There is nothing meaningful left to trim.

If you have changed any file to work around a build problem, revert it now:
...
Subsequent builds are much faster because Docker caches the dependency layer.

**EXPECTED:** the build ends with `naming to docker.io/library/catdog-api:local`, and
`docker images` lists `catdog-api` with tag `local`, size roughly 1.4–2.5 GB.
